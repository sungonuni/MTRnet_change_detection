...
from utils.helpers import (get_loaders, get_criterion,
                           ...
                           set_metrics)
from overlapTileUtils.overlapTilehelpers import split, split4label
...
for epoch in range(opt.epochs):
    ...
    for batch_img1, batch_img2, labels in tbar:
        ...
        # Make quarter splited batch img (split module) / recommand batch size 4
        splited_img1 = split(batch_img1, crop_size)
        splited_img2 = split(batch_img2, crop_size)
        splited_label = split4label(labels, crop_size)

        splited_img1 = splited_img1.float().to(dev)
        splited_img2 = splited_img2.float().to(dev)
        splited_label = splited_label.long().to(dev)
        ...
        cd_loss = criterion(splited_preds, splited_label) # must set get_criterion to 'hybrid'
        loss = cd_loss
        loss.backward()
        optimizer.step()

        # Calculate and log other batch metrics
        cd_preds = splited_preds[-1]
        _, cd_preds = torch.max(cd_preds, 1)

        cd_corrects = (100 *
                       (cd_preds.squeeze().byte() == splited_label.squeeze().byte()).sum() /
                       (splited_label.size()[0] * (opt.patch_size**2)))

        cd_train_report = prfs(splited_label.data.cpu().numpy().flatten(),
